...
const CustomerDetails = () => {
  ...
  const fetchCustomers = async (query) => {
    try {
      const response = await fetch('http://egg-8v9i.onrender.com/customers/egg-bucket-b2b/getAllCustomer'+query);
  ...
  const handleSaveEdit = async (formData) => {
    try {
      const response = await fetch(`http://egg-8v9i.onrender.com/customers/egg-bucket-b2b/customer/${editingCustomer._id}`, {
        method: 'PATCH',
        body: formData
      });
      const data = await response.json();
  ...
  const handleDeleteClick = async (Id) => {
    ...
    if (confirmDelete) {
       ...
       console.log(Id)
      try {
        const response = await fetch(`http://egg-8v9i.onrender.com/customers/egg-bucket-b2b/customer/${Id}`, {
          method: 'DELETE'
        });
